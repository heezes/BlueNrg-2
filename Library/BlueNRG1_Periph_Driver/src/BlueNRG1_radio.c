...
#define BLE_STATUS_AWS_SUCCESS            		    (0x00)
...
* @param  wakeupTime
* @retval uint8_t: BLE_STATUS_AWS_SUCCESS or NO_DEEP_SLEEP
*/
...
void RADIO_IRQHandler(void)
{
  ...
  if ( (int_value & IRQ_DONE) != 0) {
    ...
    (bluedata + next->StateMachineNo)->remap_chan = (bluedata+next->StateMachineNo)->remap_chan | 0x80;
     ...
     /*   if(timerSelect(next->WakeupTime) == BLE_STATUS_AWS_SUCCESS) {
...
{
  uint8_t returnValue = AWS_SUCCESS_0;
  uint32_t dummyTime ;
